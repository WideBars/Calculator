package dev.widebars.math.extensions
...
import dev.widebars.math.helpers.Config
import dev.widebars.math.helpers.MyWidgetProvider
import dev.widebars.math.interfaces.CalculatorDao
...
fun Context.getStrokeColor(): Int {
    return if (isDynamicTheme()) {
        if (isSystemInDarkMode()) {
            resources.getColor(dev.widebars.commons.R.color.md_grey_800, theme)
        } else {
            resources.getColor(dev.widebars.commons.R.color.md_grey_400, theme)
        }
    } else {
        val lighterColor = getProperBackgroundColor().lightenColor()
        if (lighterColor == Color.WHITE || lighterColor == Color.BLACK) {
            resources.getColor(dev.widebars.commons.R.color.divider_grey, theme)
        } else {
            lighterColor
        }
    }
